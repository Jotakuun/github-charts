  ...
  createBackgroundSegments() {
    ...
    for (let i = 0; i < config.levels; i++) {
      ...
      g.selectAll(".levels")
        ...
        .enter()
        .append("svg:line")
        .attr("x1", ((d, i) => levelFactor * (1 -  Math.sin(i * 2 * Math.PI / test_data.length)) ))
        .attr("y1", ((d, i) => levelFactor * (1 -  Math.cos(i * 2 * Math.PI / test_data.length)) ))
        .attr("x2", ((d, i) => levelFactor * (1 -  Math.sin((i + 1) * 2 * Math.PI / test_data.length)) ))
        .attr("y2", ((d, i) => levelFactor * (1 -  Math.cos((i + 1) * 2 * Math.PI / test_data.length)) ))
